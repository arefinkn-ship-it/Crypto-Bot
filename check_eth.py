...
print("=" * 60)
print("🔍 CHECKING ETHUSDT - MULTI-TIMEFRAME ALIGNMENT")
print("=" * 60)

conn = sqlite3.connect('crypto_bot.db')

# Load data for ETHUSDT
df_5m = pd.read_sql("SELECT * FROM price_data WHERE symbol='ETHUSDT' AND timeframe='5m' ORDER BY timestamp DESC LIMIT 300", conn)
df_h1 = pd.read_sql("SELECT * FROM price_data WHERE symbol='ETHUSDT' AND timeframe='1h' ORDER BY timestamp DESC LIMIT 200", conn)
df_15m = pd.read_sql("SELECT * FROM price_data WHERE symbol='ETHUSDT' AND timeframe='15m' ORDER BY timestamp DESC LIMIT 200", conn)
conn.close()
...
if len(df_5m) < 200 or len(df_h1) < 30 or len(df_15m) < 30:
    print("\n❌ Not enough data for ETHUSDT yet.")
    print("   Need 200+ 5m, 30+ H1, 30+ 15m candles")
    print(f"   Current: 5m={len(df_5m)}, H1={len(df_h1)}, 15m={len(df_15m)}")
    sys.exit(0)
...
print(f"EMA20: {h1_trend.get('ema_fast', 0)}")
print(f"EMA50: {h1_trend.get('ema_slow', 0)}")
print(f"Price: {h1_trend.get('price', 0)}")
print(f"Reason: {h1_trend['reason']}")

# ===== 15m TREND =====
print("\n" + "=" * 60)
print("📊 15m TREND (ETHUSDT)")
print("=" * 60)
...
print(f"EMA20: {m15_trend.get('ema_fast', 0)}")
print(f"EMA50: {m15_trend.get('ema_slow', 0)}")
print(f"Price: {m15_trend.get('price', 0)}")
print(f"Reason: {m15_trend['reason']}")

# ===== STRATEGY SCORES =====
print("\n" + "=" * 60)
print("📊 STRATEGY SCORES (5m - ETHUSDT)")
print("=" * 60)
...
print("\n" + "=" * 60)
print("📊 MULTI-TIMEFRAME SIGNAL (ETHUSDT)")
print("=" * 60)
...
if mt_signal['direction'] != 'NEUTRAL':
    print(f"✅ ETHUSDT HAS ALIGNED SIGNAL: {mt_signal['direction']} (Score: {mt_signal['score_boosted']:.2f})")
    if mt_signal['score_boosted'] >= 7.0:
        print("   ✅ Score is high enough for Telegram alert!")
        print("   → Run the bot to receive alerts")
    else:
        print(f"   ❌ Score {mt_signal['score_boosted']:.2f} < 7.0 - try lowering MIN_SIGNAL_SCORE")
elif combined['direction'] != 'NEUTRAL' and mt_signal['direction'] == 'NEUTRAL':
    print(f"❌ Multi-timeframe filter BLOCKED {combined['direction']} signal on ETHUSDT")
    print(f"   Reason: {mt_signal['reason']}")
else:
    print("⏸️ No signal from 5m strategies on ETHUSDT")
